...
const App: React.FC = () => {
  ...
  const [showGatoModal, setShowGatoModal] = useState(false);
  ...
  // FIRST RUN CHECK
  useEffect(() => {
    const hasAcceptedLegal = localStorage.getItem('gh_legal_accepted');
    if (!hasAcceptedLegal) {
      // Verzögerung, damit es nach dem Intro sauber da ist (oder direkt dahinter liegt)
      setTimeout(() => setShowGatoModal(true), 1500);
    }
  }, []);
  ...
  const handleConfirmLegal = useCallback(() => {
    localStorage.setItem('gh_legal_accepted', 'true');
  ...
  return (
    <div className="h-screen w-screen flex flex-col bg-black text-white select-none overflow-hidden font-inter relative">
      ...
      {/* --- GATO DYNAMICS CI MODAL --- */}
      {showGatoModal && (
        <div className="fixed inset-0 z-[1000] bg-black/95 backdrop-blur-2xl overflow-y-auto animate-in fade-in zoom-in duration-300">
          <div className="min-h-screen w-full flex flex-col items-center justify-start py-12 px-4 relative">
            <button 
              onClick={() => setShowGatoModal(false)}
              className="fixed top-6 right-6 w-12 h-12 flex items-center justify-center bg-neutral-900 rounded-full border border-neutral-800 active:scale-90 z-[1010] shadow-xl hover:bg-neutral-800"
            >
            ...
            <div className="w-full max-w-md flex flex-col items-center mt-8">
               ...
               <h2 className="font-sci-fi text-3xl font-black text-white italic tracking-tighter leading-none text-center">GATO DYNAMICS</h2>
               ...
               <div className="w-full space-y-3">
                  <div className="bg-[#0098d4]/10 border border-[#0098d4]/30 p-3 rounded-xl text-center">
                     <div className="text-[10px] font-black text-white uppercase tracking-widest mb-1">
                        Bavarian Wood Edition
                     </div>
                     <p className="text-[9px] text-[#0098d4] font-bold uppercase tracking-wide">
                        Offline-Optimized Core.
                     </p>
                     <p className="text-[8px] text-neutral-500 uppercase mt-0.5 tracking-wider">
                  ...
                  <div className="bg-neutral-900/50 border border-neutral-800 p-6 rounded-[24px] text-[9px] text-neutral-500 font-medium text-left leading-relaxed mt-2">
                    <h4 className="text-white font-black uppercase mb-3 text-[10px] tracking-widest border-b border-neutral-800 pb-2">Rechtliche Hinweise & Urheberrecht</h4>
                    
                    <p className="mb-2 uppercase font-bold text-neutral-400">
                      1. Geistiges Eigentum:
                    </p>
                    <p className="mb-4 uppercase text-[9px] leading-relaxed">
                      Sämtliche Inhalte dieser Applikation, insbesondere Quellcode, Design-Elemente und Algorithmen, sind ausschließliches geistiges Eigentum von <span className="text-white">Gato Dynamics</span>.
                    </p>
                    
                    <p className="mb-2 uppercase font-bold text-neutral-400">
                      2. Lizenz & Haftungsausschluss:
                    </p>
                    <p className="mb-4 uppercase text-[9px] leading-relaxed">
                    ...
                    <div className="pt-2 mt-2">
                      <p className="mb-2 uppercase font-bold text-neutral-400">
                        3. Sonderstatus & Nutzungsrechte (Green Hornets Landshut):
                      </p>
                      <p className="uppercase text-[9px] leading-relaxed">
                        Abweichend von den Bestimmungen in Ziffer 1 und 2 wird dem "Paintball Team Green Hornets Landshut" (nachfolgend "Lizenznehmer") ein uneingeschränktes, jedoch nicht übertragbares Nutzungs- und Verwertungsrecht an der gegenständlichen Applikation eingeräumt. Der Lizenznehmer ist den Eigentümern gleichgestellt. Das hier eingeräumte Sonderrecht ist jedoch personengebunden an das Kollektiv "Green Hornets Landshut". Eine Übertragung, Lizenzierung oder Veräußerung von Eigentums-, Nutzungs- oder Verwertungsrechten an Dritte Parteien ist ausgeschlossen und bleibt alleiniges Vorrecht von Gato Dynamics.
                      </p>
                    </div>

                    <div className="text-center border-t border-neutral-800 pt-3 mt-4 flex items-center justify-center gap-2 text-white uppercase italic tracking-widest">
                      <GatoLogo className="w-5 h-5 inline-block" />
                      <span>© {new Date().getFullYear()} GATO DYNAMICS. ALL RIGHTS RESERVED.</span>
                    </div>
                  </div>

                  <button 
                    onClick={handleConfirmLegal}
                    className="w-full px-10 py-4 mt-4 bg-[#0098d4] text-black font-black uppercase text-xs rounded-2xl shadow-[0_0_20px_rgba(0,152,212,0.4)] active:scale-95 transition-transform"
                  >
