...
const Stats: React.FC<StatsProps> = ({ transactions, teams, paymentMethods, expenses, products, categories, lootConfig }) => {
  ...
  if (lootConfig.active) {
    transactions.forEach(tx => {
      // MATH FIX: Wir zählen JETZT AUCH internen Paint-Verbrauch.
      // Grund: Auch wenn Refs Paint verschießen (Umsatz 0 bzw. neutralisiert), 
      // entstehen dem Betreiber Einkaufskosten für diese Paint!
      // Vorher wurde hier 'if (isInternal) return' gemacht, was falsch war.
  ...
  // PROFIT FORMULA: Umsatz - Ausgaben(Bar) - InternerVerbrauch(Verkaufswert) - Wareneinsatz(Paint) - Wareneinsatz(Essen) - Miete
      ...
      content = (
        <>
          ...
          <div className="bg-neutral-900/50 rounded-2xl p-6 border border-neutral-800 space-y-4 font-mono text-sm">
             ...
             <div className="flex justify-between text-neutral-400 italic"><span>(-) Miete (Kalk.)</span><span>-{formatPrice(rentCostForBalance)}</span></div>
          ...
          <Actions 
             onPrint={() => handlePrint("GEWINN RECHNUNG", `
               ...
               <table style="width:100%">
                 ...
                 <tr><td>Miete</td><td class="amount">-${formatPrice(rentCostForBalance)}</td></tr>
               </table>
             `)}
